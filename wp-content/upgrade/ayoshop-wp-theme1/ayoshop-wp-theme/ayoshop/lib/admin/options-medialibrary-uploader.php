...
if ( is_admin() ) {
	...
	$of_page= 'genesis_page_options-framework';
...
function optionsframework_mlu_js () {
	...
	wp_register_script( 'of-medialibrary-uploader', CHILD_THEME_OPTIONS_URL .'js/of-medialibrary-uploader.js', array( 'jquery', 'thickbox' ) );
...
function optionsframework_medialibrary_uploader( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0, $_name = '') {
	...
	$output .= '<input id="upload_' . $id . '" class="upload_button button" type="button" value="' . __( 'Upload', 'ayo' ) . '" rel="' . $int . '" />' . "\n";
...
function optionsframework_mlu_modify_tabs ( $tabs ) {
	$tabs['gallery'] = str_replace( __( 'Gallery', 'ayo' ), __( 'Previously Uploaded', 'ayo' ), $tabs['gallery'] );
	return $tabs;
}
