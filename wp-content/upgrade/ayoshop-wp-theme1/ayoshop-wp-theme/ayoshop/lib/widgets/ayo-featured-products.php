...
/** A simple text block **/
class Ayo_Featured_Products extends AQ_Block {
	...
	function __construct() {
		...
		//create the block
		parent::__construct( 'Ayo_Featured_Products', $block_options );
	}
	...
	function block($instance) {
		...
		if ( $products ) {
			if ( genesis_site_layout() == 'full-width-content' ) {
				echo do_shortcode('[featured_products per_page="'. $products .'" columns="4" orderby="date" order="desc"]');
			} else {
				echo do_shortcode('[featured_products per_page="'. $products .'" columns="3" orderby="date" order="desc"]');
			}
		}
	}
...
aq_register_block( 'Ayo_Featured_Products' );